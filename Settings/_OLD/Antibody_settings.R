source(file="Carriers/Simplified/General_settings.r")

#Ac225 recoil production
#       {Ac,  Fr,  At,  Bi,  Po,  Tl,  Pb,  Bi-stable} -> producer
R    = c(0,   0,   0,   0,   0,   0,   0,   0, #Ac
         1,   0,   0,   0,   0,   0,   0,   0, #Fr
         0,   0,   0,   0,   0,   0,   0,   0, #At
         0,   0,   0,   0,   0,   0,   0,   0, #Bi
         0,   0,   0,   0,   0,   0,   0,   0, #Po
         0,   0,   0,   0,   0,   0,   0,   0, #Tl
         0,   0,   0,   0,   0,   0,   0,   0, #Pb
         0,   0,   0,   0,   0,   0,   0,   0) #Bi-stable
R    = matrix(R, ncol = length(tau), byrow = T)

# {B, LI, KI, SP, HE, LU, PA, IN, ST, TE, ME, BO, BM, BR, TU, WB, CL}
N[,,] = 0

#Number of carriers over time
PS[1,1]        = A0*tau[1]/log(2) #Blood
t5min          = round(5*60/dt) + 1
PS[t5min,1]    = PS[1,1]*.10 #Blood
PS[t5min,2]    = PS[1,1]*.45 #Liver
PS[t5min,15]   = PS[1,1]*.35 #Clearance
PS[t5min,16]   = PS[1,1]*.10 #Tumour
PS[t5min*4,1]  = PS[1,1]*.10 #Blood
PS[t5min*4,2]  = PS[1,1]*.45 #Liver
PS[t5min*4,15] = PS[1,1]*0   #Tumour
PS[t5min*4,17] = PS[1,1]*.45 #Clearance

for (i in 1:length(OrgV)) {
  PS[1:t5min,i] = approx(x = c(1,t5min), y = c(PS[1,i],PS[t5min,i]), n=t5min)$y
  PS[(t5min+1):length(t_seq),i] = PS[t5min,i]
}

#Number of isotopes per carrier over time
PSi[1,] = c(1, 0, 0, 0, 0, 0, 0, 0)
PSi[1,] = PSi[1,]/sum(PSi[1,])

res  = Carrier.concentration(PSi, NR, tau, dfr, R, t_seq)
PSi  = res$PSi
NR   = res$NR

# plot(t_seq/3600, PS[,1], type="l", xlab="h", ylab="Normalized carrier content")
# plot(t_seq/3600, rowSums(NR), type="l", xlab="h", ylab="Normalized recoil production/s")

#Carrier biostability (half-life)
PSdeg= Inf

#Carrier internalization
PSint= 1